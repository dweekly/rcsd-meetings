...
| Meeting summaries (from transcripts) | `data/meeting-summaries.json`, `data/meeting-summaries-es.json`, `data/meeting-summaries-provenance.json` | For meetings that have been recorded, `scripts/generate-transcript-summaries.mjs` writes the card summary from the **transcript** (claude-sonnet-4-6), so it reports what the board decided rather than what the agenda listed — with the ranked agenda sent alongside to identify the items being voted. A transcript carries outcomes but not the figures in the board packets (measured: 6 of 37 published dollar amounts appear in the transcripts), so the generator refuses to overwrite a summary it did not write unless that meeting's date is named outright. `meeting-summaries-provenance.json` records source, model, transcript key and a hash of the summary for every generated entry; the agenda-based generator refuses to regenerate anything marked transcript-derived. Labeled as AI-generated wherever shown |
| Meeting summaries | `data/meeting-summaries.json`, `data/meeting-summaries-es.json` | One preview paragraph per meeting (EN + ES from a single Claude request), generated by `scripts/generate-meeting-summaries.mjs` from agenda item titles. Items are ordered for the model by the time the official agenda allocates to each section — the district's own ranking of what a meeting is about — so a scheduled staff report leads and the consent calendar is compressed into one clause with a count rather than itemized. Durations are parsed by `scripts/lib/agenda-duration.mjs` and turned into per-item weights by `scripts/lib/agenda-weight.mjs`; agendas that state no times fall back to ranking on substance. Labeled as AI-generated wherever shown |
| School profiles | `data/schools.json` | CDE enrollment, CAASPP, SARC, IRS 990 PTO filings |
| Student clubs | `data/school-clubs.json` | **AI-extracted, best-effort** list of student clubs/activities each school named during its annual board site presentation, built by `scripts/extract-school-clubs.mjs` (claude-sonnet-4-6 over the meeting transcripts, last 3 years). Every club carries an evidence quote + the meeting date(s) mentioned. Labeled not-authoritative — it reflects only what a principal said aloud, not a complete roster |
| Site presentations | `data/site-presentations.json` | The last 3 years of board "site presentation" decks per school, built by `scripts/build-site-presentations.mjs` from `document-index.json` (one deck per school-year, attributed to the title-primary school). Decks only on BoardDocs are mirrored to R2 `board-packets/{date}/` so every link is a stable `data.rcsd.info` URL. Rendered on `/schools/` + `/escuelas/` |
...
| Governance calendar | `extract-governance-calendar.py` + `translate-governance-calendar.mjs` | `data/governance-calendar.json` | Planned topics per scheduled meeting, from the district's Schedule of Agenda Items (a board-packet PDF). Read with PyMuPDF so the document's own edit marks are honoured: rows struck through are deletions and are excluded, rows in red are additions and are kept — neither is visible to flattened text extraction. Spanish is machine-translated and cached. Fresh as of 2026-09-07. |
...
## Text & Voice Line Compliance

The rcsd.info info line ((650) 482-8912 English / (650) 399-7203 español) runs on carrier-registered A2P 10DLC messaging. The compliance pages it depends on (/privacy/, /terms/, /privacidad/, /terminos/ and the homepage CTA) are generated by `scripts/build-legal.mjs` and `scripts/build-homepage.mjs`. The full carrier-approval playbook — exact-wording requirements, rejection codes, WhatsApp/WABA setup — is documented in [plugin/skills/a2p-campaign/SKILL.md](plugin/skills/a2p-campaign/SKILL.md) (fresh as of 2026-07-21).

SMS and WhatsApp on both lines are answered by a self-hosted Cloudflare Worker, [workers/text-agent/README.md](workers/text-agent/README.md) — architecture, deploy, webhook cutover/rollback, and a 104-question eval harness with a tool-verified judge (fresh as of 2026-07-21). Voice remains on Synthflow, calling the MCP server at mcp.rcsd.info directly.
