...
| Warrant registers | `data/warrant-pdf-manifest.json`, `data/warrants/{YYYY-MM}.json`, [`WARRANTS.md`](WARRANTS.md) | Monthly board-ratified warrant (check) registers, Mar 2020 – present; backfilled from BoardDocs `$file` URLs + Simbli, parsed (QSS + Escape formats, OCR fallback) into per-check line items with checksum reconciliation, for vendor-spend reporting. In progress — see [`WARRANTS.md`](WARRANTS.md) |
