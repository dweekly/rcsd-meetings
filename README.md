...
| Warrant registers | `data/warrant-pdf-manifest.json`, `data/warrants/{YYYY-MM}.json`, `data/warrants-index.json`, `data/warrant-vendor-aliases.json`, [`WARRANTS.md`](WARRANTS.md) | Monthly board-ratified warrant (check) registers, Mar 2020 – present; backfilled from BoardDocs `$file` URLs + Simbli, parsed (QSS + Escape formats, OCR fallback) into 30k+ per-check line items, each reconciled against the register's printed total. `scripts/build-warrants-db.mjs` builds a local SQLite (`warrants.db`, gitignored — carries individual names); `scripts/report-vendor-spend.mjs "<vendor>"` reports spend by fiscal year. Provenance + data caveats in [`WARRANTS.md`](WARRANTS.md) |
