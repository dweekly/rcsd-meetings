...
async function extractNight(getClientFn, night) {
  ...
  if (!FORCE && existsSync(cachePath)) return JSON.parse(readFileSync(cachePath, 'utf-8'));
...
${text}`;
  ...
  writeFileSync(cachePath, JSON.stringify(clubs, null, 2) + '\n');
...
async function main() {
  ...
  const out = {
    _metadata: {
      ...
      method: `AI-extracted from board-presentation transcripts via ${MODEL}. BEST-EFFORT, NOT AUTHORITATIVE: reflects only clubs a principal named aloud on their presentation night, not a complete school club roster. Every club carries an evidence quote and the meeting date(s) it was mentioned.`,
      model: MODEL,
