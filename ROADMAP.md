...
- [ ] **"Dial the District" voice agent** — two phone numbers (English and Spanish) that parents can call, text, or WhatsApp to ask questions about the district. **In progress 2026-07-19.** Plan: numbers owned in Twilio (vendor-portable); Synthflow voice agents (EN/ES) + Synthflow chat agents for SMS/WhatsApp via the Twilio integration; thin REST facade worker wrapping MCP-server data tools for Synthflow custom actions. Greeting must disclose AI + independent-of-district status (CA B&P §17941).
  ...
  - [x] Inbound message webhooks wired 2026-07-21 (Synthflow does NOT auto-configure these — texts got Twilio's default auto-reply until fixed): SMS via Messaging Service `MGdd84695…` Integration → "Send a webhook" (chat.synthflow.ai/webhooks/twilio/inbound + /status); WhatsApp via per-sender Messaging Endpoint Configuration, same URLs, Messaging service selector left empty (Service webhooks would override sender-level). Sender form requires "Profile about" tagline to save — set EN/ES taglines. Full playbook: `plugin/skills/a2p-campaign/SKILL.md`. **Awaiting David's end-to-end test of SMS + WhatsApp on both numbers.**
